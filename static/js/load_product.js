	...
	$.getJSON("/category.json", function (data){
		...
		if(data_child['development_doc']){
			...
			$.each(data_child['development_doc'],function(k,v){
				development_content += '<div class="col-sm-4"><div class="card"><div class="card-body"><h5 class="card-title">QingStor '+v.example_title+'</h5><p class="card-text">'+v.example_content+'</p><a target="_blank" href="'+v.doc_url+'" class="card-link">了解更多 →</a></div><a href="#" class="pdf-download-btn"><i class="fa fa-download"></i>PDF</a></div></div>';
			});
