...
2. 各资源告警、正常、未配置告警策略数据统计展示  
  ...
  资源概览中分别支持资源监控的 「告警中」、「正常」、「未配置」、未知四大类统计结果。用户可直观地看到所有的资源，各类监控状态下的数据占比。如下图所示：
...
4. 资源核心指标数据总和以及Top5统计展示
   ...
   在「资源使用排行」模块内，为用户统计出了主机、硬盘、EIP等核心基础资源的用量排序。如下图：

   ![](../_images/resource_rank.png)  

   目前支持的具体指标包括：

   1）主机：CPU使用率排序、内存使用率排序
