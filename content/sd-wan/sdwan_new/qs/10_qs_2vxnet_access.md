...
3. 配置**基本信息**，参数说明如下表所示。
   ...
   | 高级设置     | 勾选 **LAN 配置**。<br />网段：设置软件 vCPE 的私有 IP 地址段。<br />网关：设置网关。<br />DHCP 服务：勾选后，开启自动分配 IP 网络地址协议。<br />DHCP 起始地址：自动分配的 IP 的起始地址。<br />DHCP 结束地址：自动分配的 IP 的结束地址。<br /> | 勾选 **LAN 配置**。<br />网段：192.168.128.0/24<br />网关：192.168.128.2<br />DHCP 服务：开启 DHCP。<br />DHCP 起始地址：192.168.128.1<br />DHCP <br />结束地址：192.168.128.100 |
...
8. 参照**步骤 2**～**步骤 7**，完成 **CPE 01** 接入点的创建。

   创建 CPE 01 时，需要关注如下的配置。
   ...
   - 序列号
     ...
     -- 若需要启用**高可用**，需要购买两个光盒 CPE 设备。创建接入点时，分别输入两个序列号。
...
- 光盒设备。
  ...
  若需要添加备用设备，请绑定序列号激活设备，详细操作请参见[激活设备](../../open_access_point/equipment/30_bind_equipment) 。

- 软件 vCPE

  部署 vCPE，详细操作请参见[部署 vCPE](../../vcpe/30_deploy_script)。

  若需要添加备用设备，请绑定 License 激活设备，详细操作请参见。
