...
请参照如下操作步骤，创建一个光盒 CPE 接入点 **CPE 01（172.17.0.2/24）**，用于 **PC 01（172.17.0.3）** 的接入。
...
请参照如下操作步骤，创建一个软件 vCPE 接入点 **vCPE01（192.168.128.2/24）**，用于 **VM 01（192.168.128.2）** 的接入。
...
3. 配置接入点信息，如下表所示。
   ...
   | 部署方式     | 双机                                                         |
   | 关联连接云网 | sdwan                                                        |
   | 高级设置     | 勾选网关配置<ul><li>网段：192.168.128.0/24</li><li>网关：192.168.128.2</li><li>DHCP 服务：开启 DHCP。</li><li>DHCP 起始地址：192.168.128.1</li><li>DHCP 结束地址：192.168.128.100</li></ul> |
...
4. 在**初始化私有网络**区域，将 **IPv4 地址范围**设置为 `192.168.128.0/24`。

   <img src="/sd-wan/sdwan_new/_images/vm_vpc_create.png" style="zoom:50%;" />

5. 其他参数请根据需要进行配置。

6. 点击**立即创建**，根据提示信息完成 **VPC01（192.168.0.0/16）** 及 **Vxnet01（192.168.128.0/24）** 的创建操作。
...
3. 在**网络**区域，选择已创建的 **VPC 01** 和 **Vxnet 01**，手动分配 VM01 的 IP 地址 `192.168.128.3`。
...
5. 点击**立即购买**，根据提示信息完成云服务器 **VM01（192.168.128.3）** 的购买操作。
...
若硬件光盒 CPE 或者软件 vCPE 已激活，则在 SD-WAN 接入点列表中，可查看接入点状态处于**已激活**，设备状态处于**在线**。
...
3. 执行如下命令，配置静态路由。
   ...
   ```
   ip  route  add  172.17.0.0/24  via  192.168.128.2
   ```
...
配置 vCPE01 路由通告，使 vCPE01 业务网段与 CPE 01 互通。

> **注意**
>
> 硬件光盒 CPE 默认通告，可不配置 CPE 01 路由通告。
...
4. 点击**添加静态路由**，弹出**添加静态路由**窗口。
   ...
   | -------- | ---------------------------------------------------------- |
   | 名称     | vCPE01                                                     |
   | 目标网络 | 192.168.128.0/24                                           |
   | 下一跳   | 可选，若您的 vCPE 连接了路由器，可设置下一跳地址。         |
   | 路由通告 | 开启<br />若不开启路由通告，则 vCPE 之间无法建立网络通信。 |
