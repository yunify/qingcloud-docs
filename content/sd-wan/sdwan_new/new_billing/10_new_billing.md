...
- 停止计费
  - 包年包月计费退订时，若选择**是否销毁资源**为`是`，操作成功后，资源将同步删除，停止计费。
  - 按需计费时，当前余额不足维持下一个小时使用，服务停止并停止计费。
...
通常情况下，设置了自动续费，可不用再进行手动续费。若未设置自动续费，可在周期内通过**手动续费**延长使用周期。包年包月期间提升带宽，无论是否设置自动续费，升级费用依据是否在升级带宽前进行过**手动续费**操作有以下 3 种计费方式：

- 未**手动续费**

  > 自购买日到升级带宽期间未执行过**手动续费**操作，创建接入点时的购买时长作为一个周期，此处称为**购买周期**，升级费用按购买周期自然月剩余天数计算费用。若在自动续费的周期内提升带宽，计算时，**购买周期**为自动续费的时长。

  <b>升级费用 =[ ( 购买周期月数 * SD-WAN 每月接入带宽 / 购买周期内自然月天数 ) * 购买周期内实际未用天数 ] *（目标带宽 - 原带宽）</b>

- 已**手动续费**，且当前时间处于购买周期

  > 自购买日到升级带宽期间执行过**手动续费**操作，升级费用为以下两个周期费用之和：
  >
  > - 创建接入点时的购买时长作为一个周期，此处称为**购买周期**，升级费用按自然月剩余天数计算费用。
  > - 手动续费的时长作为一个周期，此处为**手动续费周期**，升级费用按续费的整月数计算费用。

  <b>升级费用= [ ( 购买周期月数 * SD-WAN 每月接入带宽 / 购买周期内自然月天数 ) * 购买周期内实际未用天数 ] *（目标带宽 - 原带宽）+ [ 手动续费周期月数 * SD-WAN 每月接入带宽 ] * （目标带宽 - 原带宽）</b> 

- 已**手动续费**，且当前时间处于手动续费周期

  > 当前时间处于手动续费周期内，则购买周期时间无剩余，升级费用为 0，此处仅计算手动续费周期内剩余天数所需费用。

  <b>升级费用 =[ ( 手动续费周期月数 * SD-WAN 每月接入带宽 / 手动续费周期内自然月天数 ) * 手动续费周期内实际未用天数 ] *（目标带宽 - 原带宽）</b>
...
VCPE 虚拟机接入点当前带宽上限设置为 4 Mbps，包年包月计费，购买时间为 6 月 2 日，到期时间为今年 9 月 2 日，现在需要提升上限至 8 Mbps，当前时间为 6 月 6 日。
...
- 之前未进行过手动续费
  ...
  > - 购买周期月数 = 3（月）
  > - 6 月到 9 月各为 30 天、31天、30 天，购买周期内自然月天数= 30 + 31 + 31 = 92（天）
  > - 购买周期内实际未用天数 = 88（天）

  升级费用 =[ ( 购买周期月数 * SD-WAN 每月接入带宽 / 购买周期内自然月天数 ) * 购买周期内实际未用天数 ] *（目标带宽 - 原带宽）
...
- 若提升带宽之前已手动续费两个月，现到期时间为 11月 2日，手动续费已付，现提升带宽，升级费用计算方式如下：
  ...
  升级费用= [ ( 购买周期月数 * SD-WAN 每月接入带宽 / 购买周期内自然月天数 ) * 购买周期内实际未用天数 ] *（目标带宽 - 原带宽）+ [ 手动续费周期月数 * SD-WAN 每月接入带宽 ] * （目标带宽 - 原带宽）
