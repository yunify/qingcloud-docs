...
## 不同 VPC 下 Vxnet 通信（1 对 1 ）

您可以通过建立 SD-WAN 核心网络的方式，并在 VPC 内的每个 Vxnet 中部署一个 vCPE，通过 vCPE 打通不同云平台不同 VPC 内 Vxnet 的网络通信。
...
## 不同 VPC 下 Vxnet 通信（多对多）

若需要将 VPC 下没有部署 vCPE 的
