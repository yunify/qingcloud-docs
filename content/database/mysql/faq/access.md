...
为确保外网访问的安全，MySQL Plus 外网访问安全组由系统维护。系统将根据外网地址所设置的端口自行放行安全组规则，用户无需另行配置和管理。

- 申请外网地址时，集群将自动绑定一个外网安全组，并且系统会根据外网端口（默认33306）自动放行安全组规则，无需另行配置。
...
- **外网访问**：申请外网地址后，您可以在 IP 白名单中添加跨区域云服务器公网 IP 地址，使用外网域名地址实现跨区域访问。
...
1. 设置 IP 白名单范围。
   ...
   将本地公网网段或者`0.0.0.0/0`，添加到 MySQL Plus 的白名单，允许全网段可访问数据库。详见[设置 IP 白名单](../../mysql/manual/mgt_connect/mgt_whitelist)。
