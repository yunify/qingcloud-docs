...
|  Innodb_buffer _pool_size      |  -1~       |   无论设置的值多少，最终都会上调到 innodb_buffer_pool_chunk_size * innodb_buffer_pool_instances 的整数倍。<br>- 若该值为-1，则将该参数设置为内存的60%。<br>- 若该值过大，最多只取内存的80% 。<span style="display: block; background-color: #D8ECDE; padding: 10px 24px; margin: 10px 0; border-left: 3px solid #00a971;"><b>说明</b>: <li>该参数修改后，数据库将重启。</li></span>|
...
|  Group_replication _transaction_size _limit | 0~ 2147483647 |  Group Replication 控制事务大小限值。若事务大小超过限值，事务将会回滚，并不在 Group Replication 广播。<br>- 默认值为 100000000。<span style="display: block; background-color: #D8ECDE; padding: 10px 24px; margin: 10px 0; border-left: 3px solid #00a971;"><b>说明</b>: <li>该参数仅适用于 MySQL Plus `金融版` MGR 性能的优化。</li></span>|
