...
| service        | String | 自定义服务类型。<li>取值 `set_readonly`，表示是否开关节点只读服务。 | Yes      |
| service_params | String | 自定义服务功能配置参数。<br> 取值示例 {"readonly":"ON"} <br>- `readonly` 表示是否开启节点只读。取值 `NO` 则关闭；取值 `YES` 则开启。<span style="display: block; background-color: #D8ECDE; padding: 10px 24px; margin: 10px 0; border-left: 3px solid #00a971;"><b>说明</b>: <li>该配置参数输入格式，需为 URL 编码 JSON 格式。</li></span>  | Yes |
