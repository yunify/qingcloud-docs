---
title: "扩容节点"
description: 本小节主要介绍如何扩容 Kafka 集群。 
keyword: 云计算,大数据,Kafka,Kafka 扩容节点,Kafka 在线伸缩
weight: 05
collapsible: false
draft: false
---

在集群性能不足时，可通过集群扩容功能来提高集群配置。Kafka 支持在线调整 CPU、内存、磁盘空间大小。

由于不同节点类型压力值不同，可对 `Kafka 节点`和 `客户端节点`分别进行纵向伸缩。
