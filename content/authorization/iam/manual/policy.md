...
策略分为预置策略和自定义策略、应用托管。您可以点击策略列表类型列的下拉选择筛选定位到相应的策略类型。

![图片](../../_images/policy1.PNG)

> 注：1、预置策略是 QingCloud 平台为满足您的日常管理需要而提供，无法删除和更改。   
>     &emsp;2、应用托管类策略是由 QingCloud AppCenter 应用开发者定义，使用者部署对应应用时即自动生成，也是无法自主删除和更改的。
