...
如下图所示，假设您在青云中的 VPC 使用的公网 IP 是 3.3.3.3，您自己数据中心的VPC 使用的公网 IP 是 6.6.6.6，这两个 VPC 背后各连接有两个私有网络，它们将通过 GRE 隧道连接在一起。
...
1. 参照简单的对连拓扑中 [1.在青云中的操作](#1-在青云中的操作) 的部分，创建一条到 6.6.6.6 的隧道。

2. 在 VPC 详情页的**管理配置**标签页中，点击**隧道服务** > **添加隧道规则**，新建一条到 9.9.9.9 的隧道。
...
4. 确认无误后，点击**提交**，然后点击页面上方的**应用修改**，以完成 VPC 网络的配置更新。这样就完成了在青云这端的隧道配置。如下图所示：
