---
title: "设置监控告警"
description: 本小节主要介绍如何设置 QingMR 监控告警。 
keyword: QingMR 监控告警
weight: 20
collapsible: false
draft: false
...
QingMR 集群监控告警可通过 QingCloud 云监控告警服务为集群服务器的资源和服务提供监控告警管理。当集群监控项超过阈值时触发告警，并通过短信、邮件等形式发送告警通知。

- 通过[绑定指标告警策略](#绑定指标告警策略)，您可以自定义告警规则和监控项，及时监控 QingMR 各项指标状态。
  
   - 可自定义监控项：`CPU利用率`、`内存使用率`、`磁盘使用量`、`节点服务状态`
   - 可自定义监控周期：`1分钟`、`5分钟`

- 通过[设置统一通知策略](#设置统一通知策略)，可将告警统一发送至统一配置的通知列表。
...
3. 在弹出的指标告警策略配置窗口，选择指标告警策略，并点击**提交**。
   ...
   > **说明**
   >
   > 若无可选指标告警策略，请参考[创建指标告警策略](#创建指标告警策略)进行新建。
...
设置统一通知策略后，当集群产生告警，将发送至统一配置的通知列表。
