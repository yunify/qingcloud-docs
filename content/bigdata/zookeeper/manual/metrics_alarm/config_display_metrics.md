...
|:--- |:--- |:--- |:--- |
| 服务模式  | 5分钟 | - | 监控当前节点的服务模式。<br />L 表示主节点，一般仅有一个主节点；F 表示从节点；S 表示独立节点。 |
| 响应延迟时间 | 5分钟 | ms | 监控当前节点响应延迟时间。可分别获取最小响应延迟时间、平均响应延迟时间、最大响应延迟时间。 |
| 吞吐量 | 5分钟 | MByte/s | 监控数据的请求接收数和发送响应数。 |
| 连接数 | 5分钟 | counts| 监控当前节点活跃和待处理连接数。 |
| ZNODE | 5分钟 | counts | 监控当前节点 ZNODE 数量。 |
