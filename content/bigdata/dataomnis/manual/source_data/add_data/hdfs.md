...
6. 选择 **HDFS** 数据源类型，进入配置数据源页面。
   ...
   2. 配置数据源连接信息。
      ...
      | Hadoop 高级配置       | Hadoop 相关的高级参数，比如 HA 配置。<li>集群 HA 模式时需要填写 core-site.xml 及 hdfs-site.xml 中的配置。<li>开启 kerberos 时需要包含 kerberos 相关配置。    | 

      <img src="/bigdata/dataomnis/_images/source_data_set_hdfs_connect.png" alt="配置 HDFS 数据源连接信息" style="zoom:50%;" />
