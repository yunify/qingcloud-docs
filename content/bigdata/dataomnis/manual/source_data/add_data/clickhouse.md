...
6. 选择 **ClickHouse** 数据源类型，进入配置数据源页面。
   ...
   2. 配置数据源连接信息。
      ...
      | <span style="display:inline-block;width:140px">参数</span>  | <span style="display:inline-block;width:520px">参数说明</span>  |
      | :--------- | -------------------------------------------- |
      | JDBC 连接 URL（IP 地址:端口/Database）    | JDBC 连接信息，格式为 jdbc:clickhouse://ip:port/database。 |         
      | 用户名（User Name）     | 连接数据库的用户名。                           |
      | 密码（Password）       | 连接数据库的密码。                                | 

      <img src="/bigdata/dataomnis/_images/source_data_set_clickhouse_connect.png" alt="配置 ClickHouse 数据源连接信息" style="zoom:50%;" />
