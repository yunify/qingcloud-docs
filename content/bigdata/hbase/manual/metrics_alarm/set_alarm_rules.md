---
title: "设置监控告警"
description: 本小节主要介绍如何设置 HBase 监控告警。 
keywords: HBase 监控告警
weight: 20
collapsible: false
draft: false
...
HBase 集群监控告警可通过 QingCloud 云监控告警服务为集群服务器的资源提供监控管理。当集群监控项超过阈值时触发告警，并通过短信、邮件等形式发送告警通知。

- 支持针对`CPU 利用率`、`内存使用率`、`磁盘使用量`重要指标，[设置独立通知策略](#设置独立通知策略)。

- 支持[绑定指标告警策略](#绑定指标告警策略)，自定义告警规则和监控项，及时监控 HBase 各项指标状态。
...
HBase 支持对每个节点配置指标告警策略。当业务发生异常时，会触发用户设定的告警阈值，并发送短信或邮件通知用户。
...
更多告警策略管理说明，请参见**云监控 CloudSat** 的[告警服务](../../../../../monitor_service/cloudsat/manual/alarm_service/)。
