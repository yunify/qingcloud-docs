...
高防IP支持网站业务(已完成IPC备案的网站域名)接入以及非网站业务接入，本小节为您介绍如何为创建的高防 IP 接入业务。
...
2. 根据提示信息输入参数。
   ...
   | ---------------------------------------------------------- | ------------------------------------------------------------ |
   | 关联高防 IP                                                | 用于过滤的防护 IP。                                          |
   | 网站域名                                                   | 指能够访问的网站地址的域名，例如：www.example.com，exam.example.com<br/>需要防护的网站域名已完成 ICP 备案。 |
   | 协议                                                       | 当前域名的协议，支持网站域名的传送协议 HTTP 和 HTTPS。       |
...
3. 点击**下一步**。
   ...
   <img src="../../_images/04.png" style="zoom:60%;" />
...
> - 前往接入网站所属 DNS 解析服务商控制台，将流量指向独享的高防 IP。支持 CNAME 和 A 记录接入。
...
2. 根据提示信息输入参数。
   ...
   | 协议                                                       | 当前端口号对应的 TCP 协议。                                  |
   | 转发端口                                                   | 高防 IP 的转发端口，范围为：0-65535                          |
   | 源站 IP 地址                                               | 指当前业务端口下后端服务真实的 IP 地址，或者选择用户创建的云服务器 IP 地址。 |
   | 源站端口                                                   | 指当前业务端口下后端服务真实的 IP 地址的端口，范围为：20~65535 |
...
> - 如果您的业务使用域名指向服务器地址，请前往接入域名所属 DNS 解析服务商控制台，将流量指向独享的高防 IP。支持 CNAME 和 A 记录接入。
