...
3. 选择操作后，页面将指引您为选择的操作设定权限作用范围，默认是作用于“全部资源”，如需将权限范围圈定到部分特定资源上，点选“特定资源”按钮并配置[资源标识符 QRN](../../introduction/qrn) 即可。QingCloud IAM 会根据您所选的操作自动关联出操作所需的所有资源，如下图：

    ![图片](../../_images/policya2.png)

    当选择“特定资源”作为权限范围时，您必须为必选（前面标有*的资源）类型指定 [QRN](../../introduction/qrn)，上图的“主机”即为必选类型。您可以使用 [QRN 生成器](../../introduction/qrn#qrn生成器)来指定相应的资源。
...
策略是由 JSON 格式创建的配置文件：[策略 JSON 编辑格式及参数说明](../../manual/json)
...
>
> JSON 编辑规则请参阅：[JSON 编辑格式及参数说明](../../manual/json)
