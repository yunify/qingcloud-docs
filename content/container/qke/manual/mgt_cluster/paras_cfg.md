...
| K8s apiserver EIP            | 如果希望通过公网访问 Kubernetes API Server，可在此处填写可用的 EIP，系统将会自动创建一个 LB 并绑定此 EIP。 | 集群创建后无法修改。                                         |
...
| 重命名 K8s 节点              | <li> **true**：表示使用易读的 Kubernetes 节点名，比如 master1（一号主节点），worker-p001（一号基础型工作节点），worker-s002（二号企业型工作节点）。</li><li>**false**：表示使用系统自动生成的随机节点名。</li> | 集群创建后无法修改。                                         |
...
| Docker 网桥地址              | Docker 网桥的 IP 地址和子网掩码，请按照标准的 CIDR 格式填写。默认为 `172.30.0.1/16`。 | 修改此项需要通过集群菜单依次重启所有 Kubernetes 节点（包括主节点、基础型节点、企业型节点和 GPU 节点），请在业务低谷时操作。 |
