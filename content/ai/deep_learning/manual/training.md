...
4. 使用刚才训练的权重对图片进行推理。
   ...
   **Inception v1 推理**
   ...
   <img src="../../_images/inception_v1_test.png" style="zoom:60%;" />
...
2. 训练开始后，将显示训练进度并在每个 epoch 完成后保存一次 checkpoint（每次保存占用约500MB空间，长时间训练请关注硬盘空间），如下图所示。
...
4. 将计算并显示6张人脸照片相互之间的距离，同一个人的照片，距离较近。如下图所示。
