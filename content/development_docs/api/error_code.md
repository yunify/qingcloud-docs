...
| 4003 | 超出证书授权 | 当用户的当前宿主机的数量已超过授权证书的规定数量时，会返回该错误 |
...
| 5600 | 不支持的硬盘类型 | 当用户向一个不支持加密的主机挂载加密硬盘时，会返回该错误信息。 |
...
| 8001 | 配置文件中无任何映像 | 当用户配置文件中无映像时，会返回该错误 |
| 8002 | 配置文件中映像在指定区不存在 | 当用户配置文件中的映像在指定区不存在时，会返回该错误 |
