...
针对用户长期、稳定的 IT 需求，青云 QingCloud 全新推出『预留合约』计费模式， 目前此模式仅适用于主机、专属宿主机，和关系型数据库，相对于弹性付费可享有更优惠价格。
...
![](/compute/vm/billing/_images/apply_reserved_contract.png)
...
![](/compute/vm/billing/_images/lease_new_reserved_contract.png)

## 续费合约

合约到期之前用户可以随时对合约进行续费操作，每次续费时都可以选择不同有效期。

![](/compute/vm/billing/_images/renewal_reserved_contract.png)

同样，续约操作提交之后用户可进行支付（或取消），支付完成之后合约的有效期将自动延长。

![](/compute/vm/billing/_images/lease_renewal_reserved_contract.png)
...
![](/compute/vm/billing/_images/20201014-1.jpg)

> 注：调整配置前请按照页面提示解绑资源实例。

![](/compute/vm/billing/_images/20201014-2.png)
...
![](/compute/vm/billing/_images/reserved_applyments.png)
...
![](/compute/vm/billing/_images/reserved_contract1.png)

**支付合约**

![](/compute/vm/billing/_images/lease_new_reserved_contract1.png)

**续费合约** 合约到期之前用户可以随时对合约进行续费操作，每次续费时都可以选择不同有效期。

![](/compute/vm/billing/_images/renewal_reserved_contract1.png)

同样，续约操作提交之后用户可进行支付（或取消），支付完成之后合约的有效期将自动延长。

![](/compute/vm/billing/_images/lease_renewal_reserved_contract1.png)

**升级合约**

![](/compute/vm/billing/_images/upgrade_reserved_contract1.png)
...
![](/compute/vm/billing/_images/reserved_resources1.png)

绑定成功之后该关系型数据库将不再使用“弹性计费”的模式，解绑成功之后则恢复“弹性计费”。

另外，合约详情页面也支持直接创建相应配置的新关系型数据库并直接与合约绑定。

![](/compute/vm/billing/_images/create_reserved_resources1.png)

**RDB 合约申请记录**

![](/compute/vm/billing/_images/reserved_applyments1.png)